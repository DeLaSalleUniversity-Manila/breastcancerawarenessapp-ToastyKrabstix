...
public class about extends AppCompatActivity
    ...
    private void initializeRTS(){
        rts = new TextToSpeech(this, new TextToSpeech.OnInitListener(){
            @Override
            public void onInit(int status){
                rtsLoaded = true;
            }
    ...
    private void loadPhrases(){
        ...
        Readme = new ArrayList<String>();
        Scanner scan = new Scanner(getResources().openRawResource(R.raw.phrases));
        Scanner scanz = new Scanner(getResources().openRawResource(R.raw.about));
        String waw = scanz.nextLine();
        Readme.add(waw);
    ...
    private void ReadOut(){
        String text = Readme.get(0);
        if(rtsLoaded){
            rts.setSpeechRate(1f);
            rts.speak(text, TextToSpeech.QUEUE_FLUSH,null);
        }
    }
    @Override
    ...
    private void handleIntent() {
        Intent i = getIntent();
        TextView tout = (TextView) findViewById(R.id.tv_about);
    }
    @Override
